...
api.get("/search", async (request, response) => {
    ...
    if (user == null) {
        ...
        for (let element of repos.data) {
            ...
            Object.fromEntries(Object.entries(repo).filter(entrie => allowedRepoProperties.includes(entrie[0])))
            repo.user = {
                connect: { // PK->FK
        ...
        console.log("dbRepos", dbRepos)

        const listRepos = await prisma.repos.createMany({ data: [dbRepos] })
